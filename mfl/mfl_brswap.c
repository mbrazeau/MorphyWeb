...
bool mfl_TBR_reroot(mfl_node_t* subtr, mfl_node_t* src, mfl_subtree_edges_t* stedges, mfl_cliprec_t* initial, bool neighbour_rule)
{
    ...
    if (!stedges->ste_edges) {
        ...
        if (neighbour_rule) {
            start = subtr->nodet_next->nodet_edge;
            mfl_get_all_subtree_edges(subtr->nodet_next->nodet_edge, stedges, start);
            start = subtr->nodet_next->nodet_next->nodet_edge;
            mfl_get_all_subtree_edges(subtr->nodet_next->nodet_next->nodet_edge, stedges, start);
        } else {
