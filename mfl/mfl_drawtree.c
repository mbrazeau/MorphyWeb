...
void mfl_printree_set_node_coordinates(mfl_node_t *n, int row, int num_taxa)
{
...
void mfl_printtree_set_coords_traversal(mfl_node_t* n, int* currentrow, char* grid, int num_taxa)
{
    mfl_node_t* p;
    
    if (n->nodet_tip) {
        mfl_printree_set_node_coordinates(n, *currentrow, num_taxa);
    ...
    p = n->nodet_next;
    do {
        mfl_printtree_set_coords_traversal(p->nodet_edge, currentrow, grid, num_taxa);
...
void tui_test_tree_printing()
{
    ...
    int firstrow = 0;
    mfl_printtree_set_coords_traversal(printme->treet_root, &firstrow, grid, num_taxa);
